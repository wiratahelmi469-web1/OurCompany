export const categories = [
  {
    title: "Tanaman Indoor",
    description:
      "Tanaman hias berkualitas untuk menciptakan suasana ruangan yang lebih segar, nyaman, dan elegan.",
    image: "/images/products/indoor.jpeg",
  },
  {
    title: "Tanaman Outdoor",
    description:
      "Beragam tanaman outdoor pilihan untuk mempercantik taman, halaman, dan area komersial Anda.",
    image: "/images/products/outdoor.jpeg",
  },
  {
    title: "Tanaman Kamboja Fosil & Pule",
    description:
      "Koleksi Kamboja Fosil dan Pule berkualitas dengan bentuk unik untuk taman yang lebih berkarakter.",
    image: "/images/products/kamboja&pule.jpeg",
  },
];

export const featuredProducts = [
  {
    name: "Monstera",
    category: "Tanaman Indoor",
    image: "/images/products/monstera.jpeg",
    description:
      "Daun unik dengan tampilan elegan, cocok untuk mempercantik ruangan.",
  },
  {
    name: "Anggrek",
    category: "Tanaman Premium",
    image: "/images/products/anggrek.jpeg",
    description:
      "Koleksi tanaman anggrek berkualitas dengan bunga yang indah dan warna memikat, cocok untuk mempercantik rumah, taman, maupun area komersial.",
  },
  {
    name: "Kamboja Fosil & Pule",
    category: "Koleksi Langka",
    image: "/images/products/kamboja&pule.jpeg",
    description:
      "Koleksi Kamboja Fosil dan Pule dengan karakter batang eksotis serta nilai estetika tinggi, cocok sebagai focal point untuk taman, landscape, maupun area komersial.",
  },
];

export const plantCareTips = [
  {
    title: "Menyiram dengan Tepat",
    description:
      "Siram tanaman sesuai kebutuhan. Hindari penyiraman berlebihan agar akar tetap sehat.",
  },
  {
    title: "Berikan Cahaya Cukup",
    description:
      "Pastikan tanaman mendapatkan intensitas cahaya yang sesuai dengan jenisnya.",
  },
  {
    title: "Gunakan Pupuk Berkualitas",
    description:
      "Gunakan pupuk berkualitas secara rutin untuk mendukung pertumbuhan tanaman.",
  },
];

export const whyBuyUs = [
  {
    title: "Tanaman Sehat",
    description:
      "Semua tanaman dirawat dengan baik sebelum sampai ke tangan pelanggan.",
  },
  {
    title: "Pengemasan Aman",
    description:
      "Pengemasan aman menggunakan bahan berkualitas agar tanaman tetap segar selama pengiriman.",
  },
  {
    title: "Pengiriman Cepat",
    description:
      "Pengiriman cepat ke berbagai wilayah dengan proses yang terpercaya.",
  },
  {
    title: "Dukungan Ramah",
    description:
      "Tim BioPlant siap membantu konsultasi sebelum dan sesudah pembelian.",
  },
];

export const faqItems = [
  {
    question: "Bagaimana cara memesan tanaman?",
    answer:
      "Anda dapat menghubungi kami melalui WhatsApp atau mengunjungi marketplace resmi Greenovative untuk melakukan pemesanan.",
  },
  {
    question: "Apakah Greenovative melayani pengiriman ke luar kota?",
    answer:
      "Ya. Kami melayani pengiriman ke berbagai kota di Indonesia dengan metode packing yang aman agar tanaman tetap terjaga.",
  },
  {
    question: "Apakah tanaman mendapatkan garansi?",
    answer:
      "Kami memastikan setiap tanaman dikirim dalam kondisi terbaik. Jika terdapat kendala saat pengiriman, silakan hubungi tim kami.",
  },
  {
    question: "Bagaimana cara merawat tanaman setelah diterima?",
    answer:
      "Kami akan memberikan panduan perawatan sesuai jenis tanaman agar tetap sehat dan tumbuh optimal.",
  },
];
